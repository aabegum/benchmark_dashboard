...
TUKEY_FACTOR: float = config.get('TUKEY_FACTOR', 3.0)  # New Tukey factor
START_COL = 3
...
def filtered_mean_with_outliers_tukey(row: pd.Series, start_col: int, end_col: int, tukey_factor: float = 3.0) -> dict:
    """
    Calculate the mean using Tukey's Fences outlier detection method.
    
    Tukey's Fences is similar to IQR but allows for different sensitivity levels:
    - tukey_factor = 1.5: Standard outliers (same as typical IQR)
    - tukey_factor = 3.0: Far outliers (more conservative)
    
    Parameters:
    row (pd.Series): A row from a pandas DataFrame.
    start_col (int): Starting column index.
    end_col (int): Ending column index (exclusive).
    tukey_factor (float, optional): Multiplier for IQR bounds. Default is 3.0.
    ...
    if len(valid_data) < 4:
        logger.warning(f"APG No {row.get('APG No', 'Unknown')}: Insufficient valid data for Tukey's Fences ({len(valid_data)} non-NaN values)")
        return {'filtered_mean': np.nan, 'outliers': [], 'outlier_count': 0}
    ...
    if iqr == 0 or np.isnan(iqr):
        logger.warning(f"APG No {row.get('APG No', 'Unknown')}: Zero or NaN IQR for Tukey's Fences")
        return {'filtered_mean': valid_data.mean() if not valid_data.empty else np.nan, 'outliers': [], 'outlier_count': 0}
    ...
    # Calculate filtered data and mean
    filtered_data = data[(data >= lower_fence) & (data <= upper_fence) & data.notna()]
    
    if filtered_data.empty:
        logger.warning(f"APG No {row.get('APG No', 'Unknown')}: All values filtered out (Tukey's Fences method)")
        return {'filtered_mean': valid_data.mean(), 'outliers': outliers, 'outlier_count': len(outliers)}
...
def create_powerpoint():
    ...
    for _, row in shuffled_df.iterrows():
        ...
        if row["Bulgu?"]:
            ...
            std_mean = format_value(row["filtered_mean_std"], row["Birim"])
            iqr_mean = format_value(row["filtered_mean_iqr"], row["Birim"])
            mad_mean = format_value(row["filtered_mean_mad"], row["Birim"])
            tukey_mean = format_value(row["filtered_mean_tukey"], row["Birim"])  # New Tukey mean
            
            # Updated Bulgu text to include all four methods
            bulgu_text = (
                f"Bulgu\n{NUM_OF_COMPANIES} şirketin ortalaması:\n"
                f"- STD: {std_mean} (Aykırı: {row['outlier_count_std']})\n"
                f"- IQR: {iqr_mean} (Aykırı: {row['outlier_count_iqr']})\n"
                f"- MAD: {mad_mean} (Aykırı: {row['outlier_count_mad']})\n"
                f"- Tukey: {tukey_mean} (Aykırı: {row['outlier_count_tukey']})"  # New Tukey line
            )
...
if __name__ == "__main__":
    ...
    for company_group, company_list in COMPANY_GROUPS.items():
        ...
        # NEW: Tukey's Fences method
        tukey_results = shuffled_df.apply(
            lambda row: filtered_mean_with_outliers_tukey(row, START_COL, END_COL, TUKEY_FACTOR),
            axis=1
        )
        shuffled_df['filtered_mean_tukey'] = tukey_results.apply(lambda x: x['filtered_mean'])
        shuffled_df['outliers_tukey'] = tukey_results.apply(lambda x: x['outliers'])
        shuffled_df['outlier_count_tukey'] = tukey_results.apply(lambda x: x['outlier_count'])
        ...
        report_columns = [
            ...
            "filtered_mean_tukey", "outlier_count_tukey", "outliers_tukey"  # New Tukey columns
        ]
