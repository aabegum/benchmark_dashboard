...
def consensusgraph(row: pd.Series) -> plt.Figure:
    """
    Create a scatter plot with different markers for consensus outliers.
    
    Regular companies: circles
    Consensus outliers: red X markers with larger size
    Company group: different colors as usual
    """
    fig, ax = plt.subplots(figsize=(10, 6))
    
    # Get consensus outlier company indices
    consensus_outliers = row['consensus_outliers']
    consensus_company_indices = set()
    
    # Extract company indices from consensus outliers list
    for company_col, _ in consensus_outliers:
        if isinstance(company_col, (int, np.integer)):
            consensus_company_indices.add(company_col)
    ...
    for company_index in COMPANIES_RANGE:
        value = row[company_index]
        if pd.notna(value):  # Only plot non-NaN values
            x_values.append(company_index)
            y_values.append(value)
            
            # Determine if this is a consensus outlier
            is_consensus = company_index in consensus_company_indices
            
            if is_consensus:
                # Consensus outliers: red X markers, larger size
                colors.append('red')
                markers.append('X')
                sizes.append(120)
            else:
                # Regular companies: use existing color scheme
                company_type_index = company_index - 1  # Convert to 0-based index
                if company_type_index < len(company_color_indicator):
                    if company_color_indicator[company_type_index] == GROUP_COMPANY_INDICATOR:
                        colors.append('cornflowerblue')  # Company group color
                    else:
                        colors.append('coral')  # Rival company color
                else:
    ...
    for i, (x, y, color, marker, size) in enumerate(zip(x_values, y_values, colors, markers, sizes)):
        ax.scatter(x, y, c=color, marker=marker, s=size, 
                  edgecolors='black' if marker == 'X' else 'none',
                  linewidth=2 if marker == 'X' else 0,
                  alpha=0.8, zorder=3)
    ...
    for company_index in COMPANIES_RANGE:
        ...
        if pd.notna(value):
            formatted_value = format_percentage(value) if row["Birim"] == "%" else str(value)
            # Offset annotations for consensus outliers to avoid overlap with X marker
            offset_y = 10 if company_index in consensus_company_indices else 5
            plt.annotate(
                text=formatted_value,
                xy=(company_index, value),
                fontsize=ANNOTATION_FONT_SIZE,
                xytext=(ANNOTATION_OFFSET_PIXELS[0], offset_y),
                textcoords="offset pixels",
                ha='center',
                fontweight='bold' if company_index in consensus_company_indices else 'normal'
            )
    
    # Set up the plot
    ax.set_xlabel('Şirket Numarası', fontsize=FONT_SIZE)
    ax.set_ylabel(row["Birim"] if row["Birim"] != "%" else "Değer", fontsize=FONT_SIZE)
    ...
    # Add IQR filtered mean line (purple horizontal line)
    ax.axhline(
        y=row["filtered_mean_iqr"],
        color=HORIZONTAL_MEAN_COLOR,
        alpha=HORIZONTAL_MEAN_ALPHA,
        linestyle='--',
        linewidth=2,
        label=f'IQR Ortalaması: {format_percentage(row["filtered_mean_iqr"]) if row["Birim"] == "%" else round(row["filtered_mean_iqr"], DEFAULT_DECIMAL_DIGITS)}'
    )
    
    # Title with outlier information
    wrapped_title = wrap_text_over_words(row['APG Full Name'], WORD_WRAP_LIMIT)
    consensus_count = len(consensus_outliers)
    title_with_info = f"{wrapped_title}\n(Konsensüs Aykırı: {consensus_count} şirket)"
    ax.set_title(title_with_info, fontsize=FONT_SIZE + 1, pad=20)
    
    # Custom legend
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='cornflowerblue', 
               markersize=8, label=f'{company_group} Şirketleri'),
        Line2D([0], [0], marker='o', color='w', markerfacecolor='coral', 
               markersize=8, label='Diğer Şirketler'),
        Line2D([0], [0], marker='X', color='w', markerfacecolor='red', markeredgecolor='black',
               markersize=10, label='Konsensüs Aykırı', markeredgewidth=2),
        Line2D([0], [0], color=HORIZONTAL_MEAN_COLOR, linestyle='--', 
               label='IQR Ortalaması')
    ]
    ax.legend(handles=legend_elements, loc='upper right', fontsize=FONT_SIZE - 1)
...
# Updated create_powerpoint() function section for graph type selection
def create_powerpoint():
    ...
    for _, row in shuffled_df.iterrows():
        ...
        # Updated graph type selection with new consensus graph
        ...
        elif grafik_tipi == "consensus":  # NEW: Consensus outlier scatter plot
            fig = consensusgraph(row)
        else:
            raise SystemExit(f"There is no function that corresponds to the {grafik_tipi} graph type. Please check the 'Grafik_tipi' column in the Excel file. Valid types: standard, stacked, overlayed, consensus")
        ...
        if row["Bulgu?"]:
            ...
            bulgu_text = (
                ...
                f"- Konsensüs Aykırı: {consensus_count} şirket"
            )
...
def create_summary_heatmap() -> plt.Figure:
    """
    Create a summary heatmap showing key APGs across all categories.
    This provides a high-level overview for executive dashboards.
    """
    # Select representative APGs (you can customize this selection)
    # For example, take the first APG from each category, or specific important ones
    categories = shuffled_df['Category No'].unique()
    summary_apgs = []
    
    for category in sorted(categories):
        category_data = shuffled_df[shuffled_df['Category No'] == category]
        # Take first APG from each category, or you can specify important APGs
        first_apg = category_data.iloc[0]
        summary_apgs.append(first_apg)
    
    if not summary_apgs:
        raise ValueError("No APGs found for summary heatmap")
    
    # Create summary dataframe
    summary_df = pd.DataFrame(summary_apgs)
    company_columns = [col for col in COMPANIES_RANGE]
    data_matrix = summary_df[company_columns].values
    apg_labels = summary_df['APG No'].tolist()
    
    # Create the plot
    fig_width = max(14, len(company_columns) * 0.6)
    fig_height = max(10, len(apg_labels) * 0.8)
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    
    # Use a general colormap suitable for mixed metrics
    im = ax.imshow(data_matrix, cmap='RdYlBu_r', aspect='auto', alpha=0.8)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax, shrink=0.8)
    cbar.set_label('Performans Değeri', fontsize=FONT_SIZE)
    
    # Set ticks and labels
    ax.set_xticks(range(len(company_columns)))
    ax.set_xticklabels(company_columns, fontsize=FONT_SIZE-1)
    ax.set_yticks(range(len(apg_labels)))
    ax.set_yticklabels([f"{apg} ({cat})" for apg, cat in zip(apg_labels, summary_df['Category No'])], 
                       fontsize=FONT_SIZE-2)
    
    # Add value annotations and outlier markers
    for i, row_idx in enumerate(range(len(summary_apgs))):
        row_data = summary_df.iloc[row_idx]
        consensus_outliers = row_data['consensus_outliers']
        consensus_cols = {col for col, _ in consensus_outliers}
        
        for j, company_col in enumerate(company_columns):
            value = row_data[company_col]
            ...
            if pd.notna(value):
                ...
                normalized_val = (value - data_matrix.min()) / (data_matrix.max() - data_matrix.min())
                text_color = 'white' if 0.3 < normalized_val < 0.7 else 'black'
                
                ax.text(j, i, text, ha='center', va='center', 
                       fontsize=FONT_SIZE-3, color=text_color)
                
                # Mark consensus outliers
                if company_col in consensus_cols:
                    circle = plt.Circle((j, i), 0.4, color='red', fill=False, linewidth=2)
                    ax.add_patch(circle)
    
    # Highlight company group
    if len(company_list) > 0:
        for j in range(len(company_list)):
            rect = plt.Rectangle((j-0.5, -0.5), 1, len(apg_labels), 
                               fill=False, edgecolor='blue', linewidth=2, alpha=0.7)
            ax.add_patch(rect)
    
    # Title and formatting
    total_consensus = summary_df['consensus_outlier_count'].sum()
    title = (f"Özet Performans Haritası - {company_group}\n"
             f"Seçili APG'ler, {len(company_columns)} Şirket, "
             f"Toplam Konsensüs Aykırı: {total_consensus}")
    
    ax.set_title(title, fontsize=FONT_SIZE+2, pad=20)
    ax.set_xlabel('Şirket Numarası', fontsize=FONT_SIZE)
    ax.set_ylabel('APG (Kategori)', fontsize=FONT_SIZE)
    
    # Grid
    ax.set_xticks(np.arange(len(company_columns))-0.5, minor=True)
    ax.set_yticks(np.arange(len(apg_labels))-0.5, minor=True)
    ax.grid(which='minor', color='white', linestyle='-', linewidth=1, alpha=0.5)
    
    plt.tight_layout()
    return fig
def heatmapgraph(category_no: str) -> plt.Figure:
    """
    Create a heatmap showing all APGs in a category vs all companies.
    
    Features:
    - Color-coded performance values (green=good, red=poor)
    - Outliers marked with special symbols
    - Consensus outliers highlighted
    - Missing data shown in gray
    - Company group highlighted with different border
    
    Parameters:
    category_no (str): The APG category to display (e.g., 'A1', 'B2')
    """
    # Filter data for the specific category
    category_data = shuffled_df[shuffled_df['Category No'] == category_no].copy()
    
    if category_data.empty:
        # Create empty plot if no data
        fig, ax = plt.subplots(figsize=(12, 8))
        ax.text(0.5, 0.5, f'Kategori {category_no} için veri bulunamadı', 
                ha='center', va='center', transform=ax.transAxes, fontsize=16)
        ax.set_title(f'Kategori {category_no} - Veri Bulunamadı')
        return fig
    
    # Prepare data matrix
    apg_labels = category_data['APG No'].tolist()
    company_columns = [col for col in COMPANIES_RANGE]
    
    # Create the data matrix
    data_matrix = category_data[company_columns].values
    
    # Create figure with appropriate size
    fig_width = max(12, len(company_columns) * 0.5)
    fig_height = max(8, len(apg_labels) * 0.6)
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    
    # Determine color scheme based on data characteristics
    # For percentages, higher is usually better (green)
    # For duration/time metrics, lower is usually better (reverse colormap)
    is_percentage = category_data['Birim'].iloc[0] == '%'
    is_duration = any(keyword in category_data['APG İsmi'].iloc[0].lower() 
                     for keyword in ['süre', 'dakika', 'saat', 'kesinti'])
    
    if is_duration:
        colormap = 'RdYlGn_r'  # Red for high (bad), Green for low (good)
    else:
        colormap = 'RdYlGn'    # Red for low (bad), Green for high (good)
    
    # Create the heatmap
    im = ax.imshow(data_matrix, cmap=colormap, aspect='auto', alpha=0.8)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax, shrink=0.8)
    cbar.set_label(category_data['Birim'].iloc[0] if category_data['Birim'].iloc[0] != '%' else 'Değer', 
                   fontsize=FONT_SIZE)
    ...
    # Set ticks and labels
    ax.set_xticks(range(len(company_columns)))
    ax.set_xticklabels(company_columns, fontsize=FONT_SIZE-2)
    ax.set_yticks(range(len(apg_labels)))
    ax.set_yticklabels(apg_labels, fontsize=FONT_SIZE-2)
    
    # Add text annotations and outlier markers
    for i, apg_idx in enumerate(range(len(apg_labels))):
        row_data = category_data.iloc[apg_idx]
        
        # Get outlier information for this APG
        consensus_outliers = row_data['consensus_outliers']
        iqr_outliers = row_data['outliers_iqr']
        mad_outliers = row_data['outliers_mad']
        
        # Create outlier lookup sets
        consensus_cols = {col for col, _ in consensus_outliers}
        iqr_cols = {col for col, _ in iqr_outliers}
        mad_cols = {col for col, _ in mad_outliers}
        ...
        for j, company_col in enumerate(company_columns):
            ...
            if pd.notna(value):
                ...
                text_color = 'white' if 0.3 < (value - data_matrix.min()) / (data_matrix.max() - data_matrix.min()) < 0.7 else 'black'
                
                # Add value text
                ax.text(j, i, text, ha='center', va='center', 
                       fontsize=FONT_SIZE-4, color=text_color, fontweight='normal')
                
                # Add outlier markers
                if company_col in consensus_cols:
                    # Consensus outlier: Red circle
                    circle = plt.Circle((j, i), 0.4, color='red', fill=False, linewidth=3)
                    ax.add_patch(circle)
                    ax.text(j, i-0.3, '●', ha='center', va='center', 
                           fontsize=8, color='red', fontweight='bold')
                elif company_col in iqr_cols or company_col in mad_cols:
                    # Single method outlier: Orange circle
                    circle = plt.Circle((j, i), 0.35, color='orange', fill=False, linewidth=2)
                    ax.add_patch(circle)
                ...
                ax.text(j, i, 'N/A', ha='center', va='center', 
                       fontsize=FONT_SIZE-4, color='gray', style='italic')
    
    # Highlight company group columns with different border
    if len(company_list) > 0:
        for j in range(len(company_list)):
            # Add thick border around company group columns
            rect = plt.Rectangle((j-0.5, -0.5), 1, len(apg_labels), 
                               fill=False, edgecolor='blue', linewidth=3, alpha=0.7)
            ax.add_patch(rect)
    
    # Title and labels
    category_name = category_data['Category No'].iloc[0]
    total_apgs = len(apg_labels)
    total_consensus_outliers = category_data['consensus_outlier_count'].sum()
    
    title = (f"Kategori {category_name} - Isı Haritası\n"
             f"{total_apgs} APG, {len(company_columns)} Şirket, "
             f"Toplam Konsensüs Aykırı: {total_consensus_outliers}")
    
    ax.set_title(title, fontsize=FONT_SIZE+2, pad=20)
    ax.set_xlabel('Şirket Numarası', fontsize=FONT_SIZE)
    ax.set_ylabel('APG Numarası', fontsize=FONT_SIZE)
    
    # Add legend
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', 
                   markersize=10, label='Konsensüs Aykırı', markeredgecolor='red', markeredgewidth=2),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', 
                   markersize=8, label='Tek Yöntem Aykırı', markeredgecolor='orange', markeredgewidth=2),
        plt.Rectangle((0, 0), 1, 1, facecolor='blue', alpha=0.3, 
                     label=f'{company_group} Şirketleri' if company_list else 'Hedef Grup')
    ]
    
    ax.legend(handles=legend_elements, loc='center left', bbox_to_anchor=(1.1, 0.5), fontsize=FONT_SIZE-1)
    
    # Rotate x-axis labels if too many companies
    if len(company_columns) > 15:
...
def create_category_heatmaps():
    """
    Create heatmaps for each APG category and save them.
    This function should be called separately or integrated into the main workflow.
    """
    graphics_save_directory = GRAPHICS_DIRECTORY / f'{REPORT_YEAR}_{REPORT_TYPE}' / company_group / 'heatmaps'
    graphics_save_directory.mkdir(parents=True, exist_ok=True)
    ...
    for category in categories:
        try:
            fig = heatmapgraph(category)
            
            # Save the heatmap
            clean_category = ILLEGAL_WINDOWS_PATH_CHARACTERS.sub('_', category)
            pic_path = graphics_save_directory / f'Heatmap_Kategori_{clean_category}.png'
            fig.savefig(pic_path, bbox_inches='tight', dpi=300)
            logger.info(f"Saved heatmap for category {category} to {pic_path.relative_to(MAIN_DIRECTORY)}")
            plt.close(fig)
            
        except Exception as e:
            logger.error(f"Error creating heatmap for category {category}: {e}")
            continue
if __name__ == "__main__":
    ...
    for company_group, company_list in COMPANY_GROUPS.items():
        ...
        create_powerpoint()
        logger.info(f"Creating category heatmaps for {company_group}")
        create_category_heatmaps()
        
        # Optional: Create overall summary heatmap (all categories combined)
        try:
            summary_fig = create_summary_heatmap()
            graphics_save_directory = GRAPHICS_DIRECTORY / f'{REPORT_YEAR}_{REPORT_TYPE}' / company_group
            summary_path = graphics_save_directory / 'Summary_Heatmap_All_Categories.png'
            summary_fig.savefig(summary_path, bbox_inches='tight', dpi=300)
            logger.info(f"Saved summary heatmap to {summary_path.relative_to(MAIN_DIRECTORY)}")
